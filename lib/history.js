...
function getHistoryAll (callback) {
  models.Note.findAll({
    attributes: [
      "id",
      ["title", "text"], // rename title to text
      ["lastchangeAt", "time"], // rename lastchangeAt to time
    ],
    order: ["lastchangeAt", "DESC"],
    limit: 200,
  }).then((notes) => {
    const notesWithUrlId = notes.map((note) => {
      return {
        ...note,
    ...
    if (config.debug) {
      logger.info('read history all success: ' + user.id)
    }
    return callback(null, notesWithUrlId)
  }).catch(function (err) {
    logger.error('read history failed: ' + err)
    return callback(err, null)
  })
...
function historyGetAll (req, res) {
  if (req.isAuthenticated()) {
    getHistoryAll(function (err, history) {
      if (err) return response.errorInternalError(res)
      if (!history) return response.errorNotFound(res)
      res.send({
        history: history
